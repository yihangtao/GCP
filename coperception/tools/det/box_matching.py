...
def associate_2_detections(detections1, detections2, iou_threshold=0.5):
    ...
    det_1_conf = detections1[:, 8]
    det_2_conf = detections2[:, 8]
    detections1 = detections1[:, 0:8]
    detections2 = detections2[:, 0:8]
    ...
    matched_conf = sum(det_1_conf[i] for i in [row[0][0] for row in matches]) + \
                  sum(det_2_conf[i] for i in [row[0][1] for row in matches])
    union_conf = sum(det_1_conf) + sum(det_2_conf) - matched_conf
    
    # Avoid division by zero
    conf_jaccard_index = matched_conf / union_conf if union_conf > 0 else 0

    return jaccard_index, conf_jaccard_index
