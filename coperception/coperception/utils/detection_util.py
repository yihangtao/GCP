...
def gated_late_fusion(ego_agent, num_agent, result, trans_matrices, box_color_map, iou_thresh=0.3, min_votes=2):
    """
    Gated Late Fusion with Consistency Voting.
    Only accept boxes that are supported by at least min_votes agents (including the sender).
    """
    
    # 1. Collect all boxes from all agents (transformed to ego frame)
    all_boxes = [] # List of {'box': box, 'score': score, 'agent_id': id, 'original_pred': ...}
    
    # Process Ego
    if len(result[ego_agent]) > 0:
        boxes_ego = np.array(result[ego_agent][0][0][0]["pred"])
        if boxes_ego.shape[0] > 0:
            # Ensure shape is (N, 4, 2)
            if boxes_ego.ndim == 2:
                boxes_ego = boxes_ego.reshape(1, 4, 2)
                
            scores_ego = result[ego_agent][0][0][0]["score"]
    ...
    for j in range(num_agent):
        ...
        boxes_j = np.array(result[j][0][0][0]["pred"])
        if boxes_j.shape[0] > 0:
             # Ensure shape is (N, 4, 2) before transforming
            if boxes_j.ndim == 2:
                boxes_j = boxes_j.reshape(1, 4, 2)
            ...
            points[0, :] = -points[0, :]
            points = points.T.reshape(-1, 4, 2)
            
            scores_j = result[j][0][0][0]["score"]
            ...
            for k in range(points.shape[0]):
                ...
                all_boxes.append({
                    ...
                    'score': scores_j[k],
                    'agent_id': j,
                    'color': box_color_map[j]
                })
            
    if len(all_boxes) == 0:
        return []

    # 2. Consistency Voting
    accepted_boxes = []
    box_colors = []
    
    # Statistics for analysis
    total_boxes = len(all_boxes)
    boxes_from_each_agent = {}
    for b in all_boxes:
        agent_id = b['agent_id']
        boxes_from_each_agent[agent_id] = boxes_from_each_agent.get(agent_id, 0) + 1
    
    # Debug: Check shapes
    # for i, b in enumerate(all_boxes):
    #     if b['box'].shape != (4, 2):
    #         print(f"DEBUG: Box {i} has weird shape: {b['box'].shape}")

    # To optimize, we can compute overlap matrix for all boxes vs all boxes.
    raw_boxes = np.array([b['box'] for b in all_boxes])
    
    # Ensure raw_boxes is (N, 4, 2)
    # If raw_boxes has weird shape (e.g. because of ragged nested sequences), this line might create an object array
    if raw_boxes.dtype == object or raw_boxes.ndim != 3:
         # Fallback or correction
         # print(f"DEBUG: raw_boxes shape {raw_boxes.shape}, dtype {raw_boxes.dtype}")
         # Attempt to fix individual boxes if possible, or just skip current frame?
         # Let's try to stack them explicitly
         try:
             raw_boxes = np.stack([b['box'] for b in all_boxes])
         except ValueError as e:
             print(f"Error stacking boxes: {e}")
             # Print shapes to debug
             shapes = [b['box'].shape for b in all_boxes]
             print(f"Box shapes: {shapes}")
             return []

    overlaps = compute_overlaps(raw_boxes, raw_boxes) # [N, N]
    ...
    for i in range(len(all_boxes)):
        ...
        for k in range(len(all_boxes)):
            if overlaps[i, k] > iou_thresh:
                supporting_agents.add(all_boxes[k]['agent_id'])
        
        votes = len(supporting_agents)
        
        if votes >= min_votes:
            accepted_boxes.append(all_boxes[i])
    
    # Print statistics
    accepted_boxes_per_agent = {}
    for b in accepted_boxes:
        agent_id = b['agent_id']
        accepted_boxes_per_agent[agent_id] = accepted_boxes_per_agent.get(agent_id, 0) + 1
    
    print(f"[Gated Fusion] Total boxes: {total_boxes}, Accepted: {len(accepted_boxes)}")
    print(f"[Gated Fusion] Boxes per agent (before voting): {boxes_from_each_agent}")
    print(f"[Gated Fusion] Boxes per agent (after voting): {accepted_boxes_per_agent}")
    print(f"[Gated Fusion] Filtered out: {total_boxes - len(accepted_boxes)} boxes")
            
    # 3. Update Ego Result
    if len(accepted_boxes) > 0:
        new_pred = np.array([b['box'] for b in accepted_boxes])
        new_score = np.array([b['score'] for b in accepted_boxes])
        box_colors = np.array([b['color'] for b in accepted_boxes])
        
        # Expand dims to (N, 1, 4, 2)
        new_pred = np.expand_dims(new_pred, axis=1)
        
        # We need to construct a valid result structure or just update result[ego_agent]
        # The late_fusion function updates result[ego_agent] in place.
        # However, result[ego_agent] originally contains only ego boxes.
        # We should overwrite it with accepted boxes.
        
        result[ego_agent][0][0][0]["pred"] = new_pred
        result[ego_agent][0][0][0]["score"] = new_score
        # 'selected_idx' is tricky because we mixed boxes. We can create dummy indices.
        result[ego_agent][0][0][0]["selected_idx"] = np.arange(len(accepted_boxes))
        
        # 4. NMS
        # For NMS we need (N, 4, 2)
        nms_pred = new_pred[:, 0]
        pick = non_max_suppression(nms_pred, new_score, 0.01)
        result[ego_agent][0][0][0]["pred"] = np.take(new_pred, pick, axis=0)
        result[ego_agent][0][0][0]["score"] = np.take(new_score, pick, axis=0)
        box_colors = np.take(box_colors, pick, axis=0)
        
    else:
        # No boxes accepted
        # Create empty result with shape (0, 1, 4, 2)
        result[ego_agent][0][0][0]["pred"] = np.zeros((0, 1, 4, 2))
        result[ego_agent][0][0][0]["score"] = np.zeros((0))
        box_colors = np.array([])
