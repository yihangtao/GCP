...
class FaFModule(object):
    ...
    def cls_loss_calculator(self, result, anchors, reg_loss_mask, reg_targets, labels, N, motion_labels=None, motion_mask=None, invert_gt=False, ego_loss_only=False, ego_agent=1, adv_method='pgd', bac_eps=0.3, simple_bac_eps=0.3):
        ...
        elif adv_method == 'simple_bac':
            # Simple BAC: Output-space weighting (originally BSI implementation)
            # This is the simplified version for backward compatibility
            if invert_gt:
                pseudo_labels = labels.bool()
                pseudo_labels = ~pseudo_labels
                pseudo_labels = pseudo_labels.float()
            else:
                pseudo_labels = labels.clone()
                
            weights = torch.ones_like(pseudo_labels)
            if ego_loss_only:
                ego_pred = result['cls'][ego_agent]
                ego_label = pseudo_labels[ego_agent]
                
                diff = torch.abs(ego_pred - ego_label)
                
                weights[ego_agent] = torch.where(diff > simple_bac_eps, 
                                                1.0 - torch.sigmoid(diff - simple_bac_eps),
                                                torch.ones_like(diff))
                
                loss_cls = torch.sum(weights * self.criterion['cls'](ego_pred, ego_label))
            else:
                all_pred = result['cls']
                diff = torch.abs(all_pred - labels)
                weights = torch.where(diff > simple_bac_eps,
                                    1.0 - torch.sigmoid(diff - simple_bac_eps),
                                    torch.ones_like(diff))
                loss_cls = torch.sum(weights * self.criterion['cls'](all_pred, pseudo_labels)) / N
            
        elif adv_method == 'bac':
            # Full BAC: This is placeholder for input-space blind region attack
            # The actual BAC attack is handled in bac_attack.py module
            # Here we just use standard PGD loss
            if invert_gt:
                labels = labels.bool()
                labels = ~labels
                labels = labels.float()
    ...
    def cls_step(self,data,batch_size,invert_gt=False,return_result=False,no_fuse=False,self_result=None,ego_loss_only=False,ego_agent=1,adv_method='pgd'):
        ...
        eps = data['eps']
        ego_agent = data['ego_agent']
        bac_eps = data.get('bac_eps', 0.3)
        simple_bac_eps = data.get('simple_bac_eps', 0.3)
        ...
        # print("result['loc'].shape",result['loc'].shape)
        # print("labels.shape",labels.shape)
        loss_cls = self.cls_loss_calculator(result,anchors,reg_loss_mask,reg_targets,labels,N,invert_gt=invert_gt,ego_loss_only=ego_loss_only,ego_agent=ego_agent,adv_method=adv_method, bac_eps=bac_eps, simple_bac_eps=simple_bac_eps)
        # print("loss cls:", loss_cls)
